...
# 上传更新后的xlct12.db数据库到服务器并配置应用使用它
LOCAL_DB_PATH="/Volumes/小鹿出题/小鹿备份/4月4 日81/zujuanwang81/xlct12.db"
...
sshpass -p "${SSH_PASS}" ssh -o StrictHostKeyChecking=no "${REMOTE_USER}@${REMOTE_HOST}" "
  ...
  # 检查是否已有xlct12.db文件，如果有则备份
  if [ -f ${NEW_DB_DIR}/${NEW_DB_NAME} ]; then
    echo '备份已有的xlct12.db数据库...'
    cp ${NEW_DB_DIR}/${NEW_DB_NAME} ${NEW_DB_DIR}/${NEW_DB_NAME}.bak_$(date +"%Y%m%d_%H%M%S")
    echo '已备份现有的xlct12.db'
  fi
...
# 2. 上传新数据库到服务器
echo "开始上传更新后的数据库文件到服务器 ${REMOTE_HOST}:${NEW_DB_DIR}/${NEW_DB_NAME}..."
...
echo "整个过程已完成！"
echo "1. 原数据库已备份"
echo "2. 更新后的xlct12.db数据库已上传到服务器"
