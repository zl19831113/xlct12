...
# 1. 检查模板文件中的所有路由引用
echo "检查所有模板文件中的路由引用..."
TEMPLATES_DIR="/var/www/question_bank/templates"
...
# 检查header.html中的所有url_for调用
echo "header.html中的url_for调用:"
grep -n "url_for" $HEADER_PATH

# 2. 创建一个简单但完整的app.py版本
echo "创建一个新的app.py版本..."
cat > $APP_PATH << 'END_APP'
import os
import re
import time
import traceback
...
# 添加upload_page路由（与header.html中的链接匹配）
...
def upload_paper():
    ...
    try:
        ...
        # 验证必填字段
        required_fields = ['name', 'region', 'subject', 'stage', 'source', 'year']
        ...
        # 创建存储目录
        upload_dir = os.path.join('/var/www/question_bank/papers')
        ...
        return jsonify({'success': True, 'message': '试卷上传成功', 'paper_id': new_paper.id})
    except Exception as e:
        print(f"试卷上传失败: {str(e)}")
        traceback.print_exc()
