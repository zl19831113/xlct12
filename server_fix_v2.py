...
# 新的download_paper函数
download_function = '''@app.route('/download_paper/<int:paper_id>')
def download_paper(paper_id):
    try:
        ...
        if re.search(pattern, file_name):
            ...
            if len(parts) == 2:
                prefix, suffix = parts
                file_pattern = f"{prefix}_[0-9]+_{suffix}"
                papers_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'uploads/papers')
...
# 查找download_paper函数的位置
start_line = -1
end_line = -1
for i, line in enumerate(lines):
    if "@app.route('/download_paper/<int:paper_id>')" in line:
        start_line = i
    elif start_line > -1 and "@app.route" in line:
        end_line = i
        break

if start_line > -1 and end_line > -1:
    # 替换函数
    new_lines = lines[:start_line] + [download_function + '\n\n'] + lines[end_line:]
    with open('app.py', 'w') as f:
        f.writelines(new_lines)
    print(f"已替换download_paper函数 (行 {start_line+1}-{end_line})")
else:
    print("未找到download_paper函数，请手动修改")