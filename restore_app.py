...
BACKUP_PATH = "/var/www/question_bank/app.py.bak_20250331121532"  # 使用之前一个好的备份
...
def restore_and_fix():
    ...
    try:
        ...
        backups = stdout.read().decode('utf-8').strip().split('\n')
        ...
        update_commands = [
            # 替换 db.case([(condition, value)], else_=0) 格式
            r"sed -i 's/db\.case(\[(.*), \([0-9]\+\)], else_=\([0-9]\+\))/db.case(\1, \2, else_=\3)/g' " + REMOTE_PATH,
            
            # 在papers_list函数中手动修复排序语句
            r"""sed -i '/Paper.year.desc(),/,/Paper.upload_time.desc()/c\
            ...
            r"""sed -i '/db.case(\[(Paper.region == .湖北., 1)]/,/Paper.upload_time.desc()/c\
            # 排序逻辑
            sorted_query = query.order_by(
                Paper.year.desc(),
                case((Paper.region == "湖北", 1), else_=0).desc(),
