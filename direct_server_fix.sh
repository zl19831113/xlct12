...
# 备份当前app.py
BACKUP_PATH="/var/www/question_bank/app.py.bak_$(date +%Y%m%d%H%M%S)"
cp /var/www/question_bank/app.py $BACKUP_PATH
echo "已创建备份: $BACKUP_PATH"

# 创建新的app.py文件
cat > /var/www/question_bank/app.py << 'EOF'
import os
import re
import time
import traceback
...
# 设置权限
chmod +x /var/www/question_bank/app.py

# 检查语法
echo "检查Python语法..."
python3 -m py_compile /var/www/question_bank/app.py
if [ $? -ne 0 ]; then
    echo "语法检查失败，恢复备份..."
    cp $BACKUP_PATH /var/www/question_bank/app.py
    exit 1
fi
